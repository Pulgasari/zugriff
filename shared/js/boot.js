...
(() => {
  ...
  } catch {} // ignore localStorage access restrictions in privacy modes
  ...
  if (!mapURL) throw new Error('[aufbau] boot script must be loaded as a classic script');

  // Rebase relative URLs against this script file
  const rebase = m => { for (const k in m) m[k] = new URL(m[k], mapURL).href; return m; };
  rebase(map.imports);

  // Inject importmap immediately
  document.currentScript.after(
    createElement('script', { type: 'importmap', textContent: JSON.stringify(map) })
  );

  // Inject <link rel="modulepreload"> tags
  const fragment = document.createDocumentFragment();
  for (const key of PRELOAD_CRITICAL) {
    const href = map.imports[key];
    if (href) fragment.appendChild(createElement('link', { href, rel: 'modulepreload' }));
  }
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
      navigator.serviceWorker.register('./sw.js', { type: 'module' })
        .catch(err => console.warn('[boot] sw registration failed', err));
    });
  }
})();
