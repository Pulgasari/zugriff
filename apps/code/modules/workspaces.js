...
// folder (File System Access), the GitHub client and the WebDAV client, plus the tree
// helpers all trees share (a one-slot cut/copy clipboard and a per-source version
// signal that bumps after a change so open folders reload). `dir` holds the active
// local root handle.
...
const workspaces = {
  ...
  webdav,            // the browser WebDAV client (signals + api)
