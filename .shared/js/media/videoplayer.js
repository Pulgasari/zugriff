// .shared/js/media/videoplayer.js
// the shared video-player engine: a single <video> driven imperatively, plus the
// Stage/Controls ui. state and transport live at module scope (a page has one
// player), so both the standalone videoplayer app and the videos app's player
// route import the same instance. loadFile() sets the clip; navigating into a
...
// transforms
loop     = signal(false),
reversed = signal(false),     // play backwards (rAF-driven, see below)
...
function drive () {
  ...
  else { v.play().catch(err => console.warn('[player] play() rejected:', err)); }
}
...
function Stage () {
  ...
  useEffect(() => {
    ...
    const onErr  = () => { console.warn('[player] media error:', v.error); playing.value = false; };
    v.addEventListener('loadedmetadata', onMeta);
    v.addEventListener('timeupdate', onTime);
    v.addEventListener('ended', onEnd);
    v.addEventListener('error', onErr);
    // re-entering a route remounts <video>: rebind the current clip + position
    if (src.value && v.src !== src.value) {
      const at = current.value;
      v.src = src.value; v.load();
...
export {
  loadFile, play, pause, togglePlay, seek, skip, stepFrame,
  toggleReverse, toggleLoop, cycleAspect, toggleCrop, toggleFlip, rotate,
  fmt, videoTransform, Stage, Controls,
};
