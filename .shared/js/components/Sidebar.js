// shared/js/components/Sidebar.js
...
import { html, Fragment } from '@aufbau/kits/preact-htm';

function Sidebar ({ open = false, onClose, class: klass = '', children }) {
  const asideClass = ['z-sidebar', open && 'is-open', klass].filter(Boolean).join(' ');

  return html`
    <${Fragment}>
      <aside class=${asideClass}>${children}</aside>
      ${onClose && html`
        <div class=${'z-sidebar-scrim' + (open ? ' is-open' : '')} onClick=${onClose}></div>`}
    </${Fragment}>`;
}
